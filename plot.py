...
class Visualizer:
    ...
    def __init__(self, df: pd.DataFrame, t_min_c: float = None, t_max_c: float = None, out_dir: str = 'output', settings_file: str = 'settings.yaml') -> None:
        ...
        try:
            ...
            print(f"Error loading settings from YAML file: {e}")
            self.all_settings = {}
    ...
    def draw_temp_circles(self, ax: plt.Axes) -> None:
        """
        ...
        for t in np.arange(np.ceil(self.tmin_c/settings['temp_step'])*settings['temp_step'], self.tmax_c+1, settings['temp_step']):
            ...
            ax.text(np.pi/2, t, f'{int(t)}°C', color=settings['ytick_colour'], fontsize=settings['ytick_fontsize'], ha='center', va='bottom', alpha=0.8)
            # °F label below X-axis
            ax.text(3*np.pi/2, t, f'{int(self.temp_c_to_f(t))}°F', color=settings['ytick_colour'], fontsize=settings['ytick_fontsize'], ha='center', va='top', alpha=0.8)

    def create_polar_plot(self, ax: plt.Axes, df: pd.DataFrame) -> None:
        """
        ...
        self.draw_temp_circles(ax)
        ax.set_theta_offset(np.pi/2)
        ax.set_theta_direction(-1)
        ax.set_xticks(np.arange(0, 2 * np.pi, np.pi / 6))
        figure_settings=settings['figure']
        ax.set_xticklabels(self.MONTH_LABELS, fontsize=figure_settings['xtick_fontsize'])
        ax.set_yticks([])
    ...
    def subplot_polar(
        ...
        fig = ax.get_figure()
        self.create_polar_plot(ax, df)

        # Move month xticklabels closer to the polar plot
        ax.tick_params(axis='x', pad=figure_settings['xtick_pad'])
        ax.set_title(title if title else '', fontsize=page_settings['title_fontsize'], pad=0, color=page_settings['title_colour'])
    ...
    def plot_polar_subplots(
        ...
        figure_settings = settings['figure']
        fig, axs = plt.subplots(num_rows, num_cols, figsize=(figure_settings['fig_width_in'], figure_settings['fig_height_in']), subplot_kw={'polar': True})
    
        # Set right margin to leave space for colorbars within the page width
        subplot_settings = settings['subplot']
        plt.subplots_adjust(left=subplot_settings['left'], right=subplot_settings['right'], hspace=subplot_settings['hspace'], wspace=subplot_settings['wspace'], top=subplot_settings['top'], bottom=subplot_settings['bottom'])
        ...
        for row in range(num_rows):
            for col in range(num_cols):
                ...
                if plot_idx < num_plots:
                    ...
                    pos = ax.get_position()
                    new_width = pos.width * 1.08
                    new_height = pos.height * 1.08
                    ...
                    self.subplot_polar(df=df_place, ax=ax, cbar=False, title=title)
                else:
